/*!
 * Pusher JavaScript Library v1.10.0-pre
 * http://pusherapp.com/
 *
...
var Pusher = function(app_key, options) {
  ...
  this.back_channel = new Pusher.BackChannel(this);

  var self = this;

  this.connection = new Pusher.Connection(this, this.key, this.options);
...
;(function() {
  ...
  BackChannel.prototype.trigger = function(event, data) {
    if(!isParamValid(event) || !isParamValid(data)) {
      throw new TypeError("You must pass an event and some data.");
    } else {
      this.pusher.connection.send(JSON.stringify({ event: event, data: data }));
      return this;
    }
  };

  Pusher.Util.extend(BackChannel.prototype, Pusher.EventsDispatcher.prototype);

  Pusher.BackChannel = BackChannel;
}).call(this);
...
;(function() {
  ...
  function Connection(pusher, key, options) {
    var self = this;
    self.pusher = pusher;
    ...
    function ws_onMessage(event) {
      ...
      if (params.event === 'pusher:connection_established') {
        ...
        if (typeof params["channel"] === "undefined") { // send_to_socket message
          self.pusher.back_channel.emit(params.event, params.data);
        } else {
